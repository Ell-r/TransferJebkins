            ...
            .DependentRules(() =>
            {
                RuleFor(x => x.Name)
                    .MustAsync(async (name, cancellation) =>
                        !await db.Countries.AnyAsync(c => c.Name.ToLower() == name.ToLower().Trim() && !c.IsDeleted, cancellation))
                ...
                .DependentRules(() =>
                {
                    RuleFor(x => x.Code)
                        .MustAsync(async (code, cancellation) =>
                            !await db.Countries.AnyAsync(c => c.Code.ToLower() == code.ToLower().Trim() && !c.IsDeleted, cancellation))
                ...
                .DependentRules(() =>
                {
                    RuleFor(x => x.Slug)
                        .MustAsync(async (slug, cancellation) =>
                            !await db.Countries.AnyAsync(c => c.Slug.ToLower() == slug.ToLower().Trim() && !c.IsDeleted, cancellation))
